...
pub fn basic_mesh(allocator : *const std.mem.Allocator, data : *[32768]u8) !std.ArrayList(vulkan.Vertex)
{
    ...
    //try result.append(.{ .pos = .{ -0.5, -0.5, 1.0 }, .color = .{ 0.0, 0.0, 1.0 } });
    //try result.append(.{ .pos = .{ 0.5, -0.5, 1.0 }, .color = .{ 1.0, 0.0, 1.0 } });
    //try result.append(.{ .pos = .{ 0.5, 0.5, 1.0 }, .color = .{ 1.0, 1.0, 1.0 } });

    //try result.append(.{ .pos = .{ 0.5, 0.5, 1.0 }, .color = .{ 1.0, 1.0, 1.0 } });
    //try result.append(.{ .pos = .{ -0.5, 0.5, 1.0 }, .color = .{ 0.0, 1.0, 1.0 } });
    //try result.append(.{ .pos = .{ -0.5, -0.5, 1.0 }, .color = .{ 0.0, 0.0, 1.0 } });
    ...
    //const uv: @Vector(3, f32) = .{0.0,0.0,1.0};
